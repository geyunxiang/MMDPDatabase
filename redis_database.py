...
class RedisDatabase:
	...
	def start_redis(self, password=''):
		...
		try:
			...
			self.cash_pool = ConnectionPool(host='127.0.0.1', port=6379, db=1, decode_responses=True)
	...
	def set_list_cash_all(self,key,value):
		rdb = Redis(connection_pool=self.cash_pool)
		rdb.delete(key)
		for i in value:
			rdb.rpush(key, i)
		rdb.save()
	...
	def set_list_cash(self,key,value):	#这里的key用list的名字之类的就可以，因为不是文件结构，所以键名不需要结构化
		rdb = Redis(connection_pool=self.cash_pool)
		rdb.rpush(key,value)
		rdb.save()
		return rdb.llen(key)

	def get_list_cash(self, key, start =0, end = -1):
		rdb = Redis(connection_pool=self.cash_pool)
