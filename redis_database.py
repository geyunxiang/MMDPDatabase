...
class RedisDatabase:
	...
	def start_redis(self, password=''):
		...
		try:
			...
			self.cashedb = StrictRedis(host='localhost', port=6379, db=1, decode_responses=True)
	...
	def set_list_all_cache(self,key,value):
		self.cashedb.delete(key)
		for i in value:
			self.cashedb.rpush(key, i)
		#self.cashedb.save()
		return self.cashedb.llen(key)

	def set_list_cache(self,key,value):	#这里的key用list的名字之类的就可以，因为不是文件结构，所以键名不需要结构化
		self.cashedb.rpush(key,value)
		#self.cashedb.save()
		return self.cashedb.llen(key)

	def get_list_cache(self, key, start = 0, end = -1):
		lst = self.cashedb.lrange(key, start, end)
		lst  = [ float(x) for x in lst ]
		return lst

	def exists_key_cashe(self, key):
		return self.cashedb.exists(key)

	def delete_key_cashe(self, key):
		value = self.cashedb.delete(key)
		#self.cashedb.save()
		return value

	def clear_cashe(self):
		self.cashedb.flushdb()
